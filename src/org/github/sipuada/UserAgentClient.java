...
public class UserAgentClient {
	...
	public boolean sendRegisterRequest() {
		...
		try {
			...
			logger.error("Could not properly create URI for this REGISTER request to {}."
					+ "\nMust be a valid domain or IP address: {}.", primaryHost, parseException.getMessage());
		 ...
		 *
		 * FIXME for now we only allow REGISTER requests passing a single
		 * hardcoded Contact header so this doesn't apply to us yet.
		 */

		return sendRequest(RequestMethod.REGISTER, username, primaryHost, requestUri, callIdHeader, cseq,
				contactHeaders.toArray(new ContactHeader[contactHeaders.size()]));
	}
	...
	public boolean sendInviteRequest(String remoteUser, String remoteHost, CallIdHeader callIdHeader) {
		...
		// TODO *IF* request is a INVITE, make sure to add the following
		// headers:
		/*
		...
		Header[] additionalHeaders = ((List<ContactHeader>) (Collections.singletonList(contactHeader)))
				.toArray(new ContactHeader[1]);

		return sendRequest(RequestMethod.INVITE, remoteUser, remoteHost, requestUri, callIdHeader, cseq,
				additionalHeaders);
	}
	...
	public boolean sendOptionsRequest(String remoteUser, String remoteHost, CallIdHeader callIdHeader,
		...
		try {
			...
			logger.error("Could not properly create URI for this OPTIONS request "
					+ "to {} at {}.\n[remoteUser] must be a valid id, [remoteHost] "
					+ "must be a valid IP address: {}.", remoteUser, remoteHost, parseException.getMessage());
			// No need for caller to wait for remote responses.
			return false;
		...
		return sendRequest(RequestMethod.OPTIONS, remoteUser, remoteHost, requestUri, callIdHeader, cseq,
				additionalHeaders);
	}

	private boolean sendRequest(RequestMethod method, String remoteUser, String remoteHost, URI requestUri,
			CallIdHeader callIdHeader, long cseq, Header... additionalHeaders) {
		try {
			...
			return sendRequest(method, requestUri, addresserUri, addresseeUri, null, callIdHeader, cseq,
					additionalHeaders);
		} catch (ParseException parseException) {
	...
	private boolean sendRequest(RequestMethod method, Dialog dialog, Header... additionalHeaders) {
		...
		return sendRequest(method, requestUri, addresserUri, addresseeUri, dialog, callIdHeader, cseq,
				additionalHeaders);
	}

	private boolean sendRequest(final RequestMethod method, URI requestUri, URI addresserUri, URI addresseeUri,
			final Dialog dialog, final CallIdHeader callIdHeader, long cseq, Header... additionalHeaders) {
		...
		try {
			...
			viaHeader.setRPort();
			final Request request = messenger.createRequest(requestUri, method.toString(), callIdHeader,
					headerMaker.createCSeqHeader(cseq, method.toString()), headerMaker.createFromHeader(from, fromTag),
					headerMaker.createToHeader(to, toTag), Collections.singletonList(viaHeader),
	...
	private void handleAuthorizationRequired(Response response, ClientTransaction clientTransaction) {
		...
		while (wwwAuthenticateHeaders.hasNext()) {
			...
			if (authNoncesCache.containsKey(toHeaderValue) && authNoncesCache.get(toHeaderValue).containsKey(realm)) {
	...
	private void handleUnsupportedMediaTypes(Response response, ClientTransaction clientTransaction) {
		...
		if (overlappingEncodings.length() > 0) {
			try {
				contentEncodingHeader.setEncoding(overlappingEncodings.toString());
		...
		while (acceptHeaders.hasNext()) {
			...
			if (qValue != -1) {
				typeSubtypeToQValue.put(String.format("%s/%s", contentType, contentSubType), Float.toString(qValue));
			}
		}
		...
		if (ResponseClass.SUCCESS == Constants.getResponseClass(statusCode)) {
			if (dialog != null) {
				try {
					...
					logger.info("{} response to INVITE arrived, so {} sent.", statusCode, RequestMethod.ACK);
	...
	private boolean putAnswerIntoAckRequestIfApplicable(RequestMethod method, String callId, Request request,
		...
		try {
			offer = SdpFactory.getInstance().createSessionDescription(new String(response.getRawContent()));
		...
		logger.info(
				"Plug-in-generated answer {} to offer {} by {} inserted into {} for {} response" + " to {} request.",
				answer.toString(), offer.toString(), sessionPlugin.getClass().getName(), RequestMethod.ACK,
	...
	private boolean handleThisRequestTerminated(ClientTransaction clientTransaction) {
		...
		case INVITE:
			...
			bus.post(new CallInvitationCanceled("Callee canceled " + "outgoing INVITE.", callId, false));
			return true;
		case BYE:
